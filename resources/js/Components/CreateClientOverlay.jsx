...
export default function CreateClientOverlay({ nama_klien, isSendingClientData, setResponsClientData }) {
    ...
    useEffect(() => {
        nama_klien.includes(name.toLowerCase())
            ? setNameExist(true)
            : setNameExist(false);
    }, [name]);
    ...
    const sendNewClient = async (postData) => {
        ...
        try {
            ...
            setIsSuccess(true);
            setResponsClientData(response.data.client_data)
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if(isSendingClientData) handleSubmit()
    ...
    return (
        <div className="h-4/5 pb-20 w-full overflow-scroll scrollbar-hide border-2 border-tertiary rounded-lg mb-5 p-3 relative">
            <div className="w-full flex flex-col justify-between items-start">
                ...
                />
                {nameExist ? (
                    <p className="-mt-4 mb-5 text-body">
                        <span className="text-body-heavy text-primary">
                            Maaf
                        </span>{" "}
                        - Client Sudah Ada!
                    </p>
                ...
                <div class="min-h-12 w-full">
                    ...
                    <textarea
                        id="message"
                        rows="5"
                        class="block p-2.5 w-full text-body text-tertiary bg-gray-50 rounded-lg border border-secondary focus:ring-tertiary focus:border-tesrtiary"
            ...
            {isLoading || isError || isSuccess && (<div className="w-full h-full bg-black bg-opacity-50 absolute top-0 left-0 flex-center">
                {isLoading && (
                    <div role="status">
                        <svg
                            aria-hidden="true"
